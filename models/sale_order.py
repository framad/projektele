...
class SaleOrderLine(models.Model):
    ...
    def rekap_sale_order(self):
        for record in self:
            self.env['projektele.rekap_so'].create({
            ...
            grouped_data = self.env['projektele.rekap_so'].read_group(
                [('id_produk', '=', record.product_template_id.id)],
            ...
            for total in record.order_id.env['projektele.rekap_so'].search([('id_produk', '=', record.product_template_id.id)]):
